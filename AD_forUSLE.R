...
Parent<-getwd() #AD tb chg
#Parent folder using default working directory in R
#AD avoid using the getwd() function to determine parent directory
#create RSWAT folder if it doesn't exist AD tb chg
wd <- paste0(Parent,"/RSWAT_trial1")
dir.create(wd)
#create Watershed delineation directory
dir.create(paste(wd,"/Watershed_delineation",sep=""))
#create directory for HRU definition
dir.create(paste(wd,"/HRU",sep=""))
tes_dir <- paste0(wd,"/temp_data/")
dir.create(tes_dir)
...
# Folder.raw <- c("C://Program Files") AD rmve
# Folder.saga <- NULL
...
# AD testonly cmment out====
# List.folder <- list.files(Folder.raw, pattern="saga_cmd.exe", recursive = TRUE,
#                           full.names=TRUE)
# if (grepl("SAGA", List.folder[1])){#AD tb corr not to test only the first element of the vector l. 69-72
#   Folder.saga <- dirname(grep("SAGA-GIS", List.folder, value = TRUE))
# } else {
#   Folder.saga <- choose.dir()
# }
Folder.saga <- "C:/OSGeo4W64/apps/saga"
...
DEM_raw_raster <- choose.files()

#file path to DEM saga grid format
DEM_sgrd <- paste(wd, "/DEM_Musi.sgrd", sep="")
...
fillsink_threshold <- as.numeric(dlgInput("Input threshold value (degree) for preprocessing", Sys.info()["f"])$res)
...
# Initiation_threshold <- as.numeric(dlgInput("Input channel network threshold (number of cells)", Sys.info()["Initiation_threshold"])$res) # AD rmove
# Initiation_threshold <- 1000000000 AD rmv
#5. Minimum size of number of basins
#Note: Minimum size of basins (unit in cells)
#  example for this study case
# min_size <- 0 #by default 0
order_threshold <- as.numeric(dlgInput("Input minimum Stahler Order threshold for channel delineation (default value:5, unitless)", Sys.info()["order_threshold"])$res)
min_size <- as.numeric(dlgInput("Input basin area threshold (default value:0, unit in cells)", Sys.info()["min_size"])$res) # AD rmv


#DEM raster preprocessing
Fill_sink_raster <- paste(tes_dir, "Fill_sink_DEM.sgrd", sep="")
# Fill_sink_tif <- paste (tes_dir, "Fill_sink_DEM.tif", sep="") #AD rmve changed into 'Clipped_fillsink_tif'
#Flow direction
Flow_direction_raster <- paste(tes_dir, "Flow_direction.sgrd", sep="")
Flow_direction_tif<- paste(tes_dir, "Flow_direction.tif", sep="")
#Catchment area
Catchment_area_raster <- paste(wd, "/Catchment_area.sgrd", sep="")

#Upslope area
Upslope_area_raster <- paste(tes_dir, "Upslope_area.sgrd", sep="")
Upslope_area_raster_reclass <- paste(tes_dir, "Upslope_area_reclass.sgrd", sep="")

#Channel network
Channel_network_raster <- paste(tes_dir, "Channel_network.sgrd", sep="")
Channel_network_vector <- paste(output_path_result_wd, "/Channel_network.shp", sep="" )
# Channel_network_vector_clipped <- paste(output_path_result_wd, "/Channel_network_clipped.shp", sep="") AD rmove
Channel_segmentID_raster <- paste(tes_dir, "Channel_ID.sgrd", sep="") # AD add
Channel_segmentID_tif <- paste0(tes_dir,"Channel_ID.tif") # AD add
Channel_network_tif <- paste (tes_dir, "Channel_network.tif", sep="")

#watershed basins
Watershed_basins_raster <- paste(tes_dir, "Watershed_basins.sgrd", sep="")

#watershed raster
# Watershed_raster.raw <- paste(tes_dir, "watershed_raster.sgrd", sep="")
...
#slope length
slope_dem_sgrd <- paste(tes_dir, "slope_dem.sgrd", sep="")
slope_dem_tif <- paste(output_path_result_wd, "/slope_dem.tif", sep="")

# Newly defined variables from untitled2.R====
Direction_raster <- paste0(tes_dir,"Flow_direct_rst.sgrd")
# t_Connectivity_raster <- paste0(tes_dir,"Flow_connect.sgrd") AD cmmntd out
# t_Flow_connectivity_tif <- paste0(tes_dir, "Flow_connect.tif") AD cmmntd out not required for now
Order_raster <- paste0(tes_dir,"Segment_order.sgrd")
Basins_raster <- paste0(tes_dir,"Basins.sgrd")
# Channel_segments <- paste0(tes_dir,"Segments.shp") AD changed into Channel_network_vector
Basins_vector <- paste0(tes_dir,"Basins.shp")
Nodes <- paste0(tes_dir,"Nodes.shp")
...
Clipped_fillsink <- paste0(tes_dir, "Upslp_Fill_sink_DEM.sgrd", sep="")
Clipped_fillsink_tif <- paste0(tes_dir,"Fillsink_DEM.tif")
...
#misc variables
upslope_vector <- paste0(tes_dir,"upslope.shp")
#--------------------------------------------------------------------------------------------
#2. HRU DEFINITION 

# 2. File path for Landuse
# NOTE: "rastername.tif" <-- diganti sesuai file landuse di folder
LU_path<-choose.files()

#3. File path for soilmap
# NOTE: "rastername.tif" <-- diganti sesuai file soilmap di folder
SM_path<-choose.files()
#4. File path for terrain analysis (slope) from DEM
DEM_path<-Clipped_fillsink_tif

#5. Setting up threshold for HRU definition (%)
# example by this study case
# unit: % area
#threshold_landuse<-5
#threshold_soil<-5
#threshold_slope<-5
threshold_landuse <- as.numeric(dlgInput("Landuse threshold area (%)", Sys.info()["threshold_landuse"])$res)
threshold_soil <- as.numeric(dlgInput("Soil threshold area (%)", Sys.info()["threshold_soil"])$res)
threshold_slope <- as.numeric(dlgInput("Slope threshold area (%)", Sys.info()["threshold_slope"])$res)
...
#7. Output data
Output_HRU <- paste(output_path_result_HRU, "/HRU_definition_table.csv", sep="")

#--------------------------------------------------------------------------------------------
#                                     PROCESS
#--------------------------------------------------------------------------------------------
#Setting up SAGA environment
myenv <- rsaga.env(path=Folder.saga) #AD tb corr
...
DEMsgrd <- rsaga.geoprocessor(lib = "io_gdal",
                              ...
                              param = list(FILES = DEM_raw_raster,
                                           ...
                                           TRANSFORM = TRUE,
                                           INTERPOL = 4), #default method (B-Spline Interpolation)
                              env=myenv)
...
# wshed_fill <- paste0(wd, "/Watershed_basins_fill.sgrd", sep="") AD add red
fillsink <- rsaga.fill.sinks(in.dem =  DEM_sgrd, 
                             out.dem = Fill_sink_raster,#output
                             out.flowdir = Flow_direction_raster, #output
                             # out.wshed = wshed_fill, #AD add temp red
                             method = "wang.liu.2006",
                             minslope = fillsink_threshold,
                             env=myenv)

# AD from Untitled2.R script line 17 onwards====
Sys.time()
rsaga.geoprocessor(lib = "ta_channels",
                   module = 5, 
                   param = list(DEM = Fill_sink_raster,#the only input
                                DIRECTION = Direction_raster,
                                # CONNECTION = t_Connectivity_raster, #not neccessary
                                ORDER = Order_raster,
                                BASIN = Basins_raster, #not necc
                                SEGMENTS = Channel_network_vector,
                                BASINS = Basins_vector,
                                NODES = Nodes,
                                THRESHOLD= order_threshold),   #THRESHOLD = ... parameters might be tweaked to adjust the detailness of the results
                   env=myenv)

# dir.create(tes_dir,"gridding_channel")
# t_Channel_order_tif <- paste0(tes_dir,"gridding_channel/Channel_network_dumm.tif")

# generate blank Channel_network_raster grid from existing grid
rsaga.geoprocessor(lib="grid_tools", module = "Reclassify Grid Values",
                   param = list (INPUT=Direction_raster,
                                 RESULT= Channel_network_raster,
                                 MIN = -2, MAX = 9,
                                 METHOD = 1, RNEW = -99999,
                                 NODATA = -99999, RESULT_NODATA_CHOICE= 1
                   ), env=myenv)

# generate blank Channel_segmentID_raster grid from existing grid AD add
rsaga.geoprocessor(lib="grid_tools", module = "Reclassify Grid Values",
                   param = list (INPUT=Direction_raster,
                                 RESULT= Channel_segmentID_raster,
                                 MIN = -2, MAX = 9,
                                 METHOD = 1, RNEW = -99999,
                                 NODATA = -99999, RESULT_NODATA_CHOICE= 1
                   ), env=myenv)
...
channel_shp@data$SEGMENT_ID <- as.numeric(as.character(channel_shp@data$SEGMENT_ID)) #factor to numeric converstion must involve conversion from factor to character beforehand 
...
pointLabel(mid_pts@coords[,1], mid_pts@coords[,2], labels=as.character(mid_pts@data$SEGMENT_ID), 
           offset=0, cex=0.75)
#   Selection interface
water_outlet_code <- NULL
dlgMessage("Look at the image to choose one of water outlet coordinate")
res <- dlgList(mid_pts@data$SEGMENT_ID, title="Choose ID code coordinate", multiple = TRUE)$res
if (length(res)==0) {
...
plot(channel_shp[channel_shp@data$SEGMENT_ID==water_outlet_code,])
