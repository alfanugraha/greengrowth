...
for(iter in 1:lusimRepetition){
  ...
  # write egoml
  lusimEgomlSimulation <- paste(lusimSimulationStep, "/4_simulation_per_regions.egoml", sep='')
...
``
  ...
  # setwd(lusimSimulationStep)
  
  # preparing factors
  lusimRasterCube <- lusimSimulationStep
  lusimListFactorStep <- list.files(lusimSimulationStepFactorsFolder, full.names=TRUE, pattern=".tif$")
  
  nFactorsStep <- length(lusimListFactorStep)
  
  lusimAliasFactorStep <- NULL
  for (a in 1:nFactorsStep) {
    paste(eval(parse(text=(paste("lusimTempStep <- substr(basename(lusimListFactorStep[", a, "]), 1, nchar(basename(lusimListFactorStep[", a, "])) - 4)", sep="")))))
    lusimAliasFactorStep <- c(lusimAliasFactorStep, lusimTempStep)
  }
  ...
  con$addTag("functor", attrs=c(name="SaveMap"), close=FALSE)
  con$addTag("property", attrs=c(key="dff.functor.alias", value="saveMap1680"))
  con$addTag("inputport", attrs=c(name="map", peerid=paste("v", nFactorsStep+1,sep="")))
  con$addTag("inputport", attrs=c(name="filename"), paste('"', lusimSimulationStep, '/raster_cube.ers"', sep=''))
  con$addTag("inputport", attrs=c(name="suffixDigits"), 2)
  con$addTag("inputport", attrs=c(name="step"), ".none")
  con$addTag("inputport", attrs=c(name="useCompression"), ".yes")
  con$addTag("inputport", attrs=c(name="workdir"), ".none")
  ...
  for (b in 1:nFactors){
    con$addTag("functor", attrs=c(name="LoadMap"), close=FALSE)
    con$addTag("property", attrs=c(key="dff.functor.alias", value=lusimAliasFactorStep[b]))
    con$addTag("inputport", attrs=c(name="filename"), paste('"', lusimListFactorStep[b], '"', sep=""))
    con$addTag("inputport", attrs=c(name="nullValue"), ".none")
    con$addTag("inputport", attrs=c(name="loadAsSparse"), ".no")
    con$addTag("inputport", attrs=c(name="suffixDigits"), 0)
    con$addTag("inputport", attrs=c(name="step"), ".none")
  ...
  con$addTag("inputport", attrs=c(name="cellType"), ".float32")
  con$addTag("inputport", attrs=c(name="nullValue"), "-9999")
  con$addTag("outputport", attrs=c(name="map", id=paste("v", nFactorsStep+1, sep="")))
  # add subtag functor for CreateCubeMap
  for (c in 1:nFactors) {
    con$addTag("functor", attrs=c(name="NumberAndNameMap"), close=FALSE)
    con$addTag("property", attrs=c(key="dff.functor.alias", value=lusimAliasFactorStep[c]))
    con$addTag("inputport", attrs=c(name="map", peerid=paste("v", c, sep="")))
    con$addTag("inputport", attrs=c(name="mapName"), paste('"', lusimAliasFactorStep[c], '"', sep=""))
    con$addTag("inputport", attrs=c(name="mapNumber"), 0)
    con$closeTag("functor")
  }
  con$closeTag("containerfunctor")
  ...
  # write egoml
  saveXML(con$value(), file=paste(lusimSimulationStep, "/2_create_raster_cube.egoml", sep=''))
  lusimCommand <- paste('"', lusimDinamicaConsole, '" -processors 0 -log-level 4 "', lusimSimulationStep, '/2_create_raster_cube.egoml"', sep="")
  system(lusimCommand)
}
